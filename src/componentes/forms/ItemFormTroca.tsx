...
export type ItemFormData = {
  titulo: string;
  descricao: string;
  preco?: number;        // também visível em troca
...
export default function ItemForm({
  ...
  function validate(): string | null {
    ...
    if ((kind === 'venda' || kind === 'troca') && (data.preco == null || isNaN(data.preco))) return 'Informe o preço.';
  ...
  async function salvarItem(kind: ItemKind, data: ItemFormData, token: string) {
    const fd = new FormData();
    fd.append('titulo', data.titulo);
    fd.append('descricao', data.descricao);
    fd.append('tipo', kind);
    if ((kind === 'venda' || kind === 'troca') && data.preco != null) fd.append('preco', String(data.preco));
    ...
    <form onSubmit={submit} className="cad-container cad-form">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          {(kind === 'venda' || kind === 'troca') && (
            <label>
              Preço estimado para troca (R$):
              <Input
                type="number"
                step="0.01"
                value={data.preco ?? ''}
